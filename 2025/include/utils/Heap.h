...
template<typename T, typename S>
requires std::derived_from<T, S> && std::derived_from<T, IComparable<S>>
Heap<T,S>::Heap() : _array(NULL), _capacity(0), _count(0), _dir(1) {  }

template<typename T, typename S>
requires std::derived_from<T, S> && std::derived_from<T, IComparable<S>>
Heap<T,S>::Heap(bool isMinHeap) : 
        _array(NULL), _capacity(0), _count(0), _dir(isMinHeap ? 1 : -1) {  }
...
void Heap<T,S>::downHeap(int index)
{
    ...
    {
        childIndex = 2 * index;
        if (childIndex + 1 <= this->_count && (this->_dir) * this->_array[childIndex]->CompareTo(this->_array[childIndex+1]) > 0)
        {
            childIndex++;
        }

        if ((this->_dir) * this->_array[index]->CompareTo(this->_array[childIndex]) > 0)
        {
